<?php 
	trait ProductModel{
		...
		public function update($id){
			...
			$conn = Connection::getInstance();
			//update ban ghi
			$query = $conn->prepare("update tbl_product set name=:name, category_id=:category_id,description=:description,content=:content,hotproduct=:hotproduct,price=:price where id=:id");
			$query->execute(array("name"=>$name,"category_id"=>$category_id,"description"=>$description,"content"=>$content,"hotproduct"=>$hotproduct,"price"=>$price,"id"=>$id));
			//neu user upload anh
			if($_FILES["img"]["name"] != ""){
				//---
				//lay anh cu de xoa
		...
		public function insert(){
			...
			$conn = Connection::getInstance();
			//update ban ghi
			$query = $conn->prepare("insert into tbl_product set name=:name, category_id=:category_id,description=:description,content=:content,hotproduct=:hotproduct,price=:price,img=:img");
			$query->execute(array("name"=>$name,"category_id"=>$category_id,"description"=>$description,"content"=>$content,"hotproduct"=>$hotproduct,"price"=>$price,"img"=>$img));		
		}
		//delete
